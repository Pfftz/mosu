...
- `src/app/page.tsx` — entry page (renders `ITSolutionMain`)
- `src/app/globals.scss` — global styles

- `src/pages/` — pages folder (variants / routes)
    - `src/pages/homes/it-solution/ITSolutionMain.tsx` — current homepage composition (renders on /)
    - `src/pages/homes/home/HomeMain.tsx` — homepage variant (legacy/default)
    - `src/pages/homes/*` — other homepage variants (creative-studio, corporate, personal-portfolio, etc.)
...
- If the task is "change layout / section order" → modify `src/pages/homes/it-solution/ITSolutionMain.tsx` (current homepage) or another variant under `src/pages/homes/*`.
